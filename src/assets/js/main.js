...
ScrollTrigger.create({
    ...
    scrub: 1,
    onUpdate: (self) => {
        const heroImgCurrentWidth = heroImgInitialWidth - self.progress * (heroImgInitialWidth - heroImgTargetWidth);

        gsap.set(heroImg, {
            width: `${heroImgCurrentWidth}px`,
        });
    },
});
...
ScrollTrigger.create({
    ...
    onUpdate: (self) => {
        if (!isAnimationPaused) {
            ...
            const frame = Math.floor(scrollDistance / pixelPerFrame) % lottieAnimation.totalFrames;
